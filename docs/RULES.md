...
These rules bind the orchestrator and ALL subagents (coder, tester, quality-checker). Read them first,
every invocation. They are summarized in CLAUDE.md; this is the full text.
...
- **D2. Git:** work on feature branches, commit frequently with clear messages, open the work for review
  via the agent loop. Never run destructive commands (git reset --hard, git clean -fd, force-push,
